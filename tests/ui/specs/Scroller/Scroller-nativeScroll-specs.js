...
describe('Scroller', function () {
	...
	describe('RTL Locale', function () {
		...
		it('should focus not on scroll right button with 5-way key and focusableScrollbar `false`', async function () {
			...
			expect(await (await ScrollerPage.button('scroll right')).isFocused()).not.toBe(true);
		});
		...
		it.skip('should Scrolling via 5-way Key with Spotlight on the scroll left button', async function () {
			...
			expect(initialHorizontalScrollThumbPosition).toBe('0');
			...
			expect((await (await ScrollerPage.getScrollThumbPosition()).horizontal) > initialHorizontalScrollThumbPosition).toBe(true);
		});
	});
});
